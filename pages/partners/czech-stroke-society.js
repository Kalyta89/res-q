...
const CzechStrokeSocietyPage = ({ hero, logo, text, buttonWebsiteLink }) => {
  return <PartnerProfilePageTemplate hero={hero} logo={logo} text={text} buttonWebsiteLink={buttonWebsiteLink} />;
};

export async function getStaticProps() {
  const pageHero = await getData("/czech-stroke-society");
  const [pageData] = await getData("/partner-logos?alt=Czech%20Stroke%20Society");
...
export default CzechStrokeSocietyPage;
