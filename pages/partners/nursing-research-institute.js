...
const NursingResearchInstitutePage = ({ hero, logo, text, buttonWebsiteLink }) => {
  return <PartnerProfilePageTemplate hero={hero} logo={logo} text={text} buttonWebsiteLink={buttonWebsiteLink} />;
};

export async function getStaticProps() {
  const pageHero = await getData("/nursing-research-institute");
  const [pageData] = await getData("/partner-logos?alt=Nursing%20Research%20Institute");
...
export default NursingResearchInstitutePage;
