...
const AmericanHeartAssociationPage = ({ hero, logo, text, buttonWebsiteLink }) => {
  return <PartnerProfilePageTemplate hero={hero} logo={logo} text={text} buttonWebsiteLink={buttonWebsiteLink} />;
};

export async function getStaticProps() {
  const pageHero = await getData("/aha-asa");
  const [pageData] = await getData("/partner-logos?alt=American%20Stroke%20Association");
...
export default AmericanHeartAssociationPage;
